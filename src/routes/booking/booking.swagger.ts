...
export const bookingPaths = {
  '/bookings': {
    get: {
      ...
      description: [
        'Supports search, per-field exact-match filters, date-range filters (on both `createdAt` and `updatedAt`), and sorting — combine as many query params as needed; they all AND together.',
        '',
        '**How to filter:** append any of the query params below, e.g.',
        '`/bookings?branch=jalandhar&booking_status=ongoing&payment_status=pending&sort_by=latest_updated`',
      ].join('\n'),
      security: jwtOrKey,
      parameters: [
        ...
        { in: 'query', name: 'search',         schema: { type: 'string',  example: 'nikhil' },                description: 'Search by user_name, user_phone, user_id or reference_id (case-insensitive, partial match)' },
        { in: 'query', name: 'booking_status', schema: { type: 'string', enum: ['ongoing', 'completed', 'cancelled_via_user', 'cancelled_by_admin_crm'] }, description: 'Filter by booking_status enum (exact match)' },
        { in: 'query', name: 'booking_via',    schema: { type: 'string', enum: ['app', 'website', 'laptop', 'whatsapp_to_crm', 'call'] },                  description: 'Filter by booking_via enum (exact match)' },
        { in: 'query', name: 'branch',         schema: { type: 'string',  example: 'jalandhar' },             description: 'Filter by branch (exact match)' },
        { in: 'query', name: 'payment_status', schema: { type: 'string', enum: ['paid', 'pending', 'cancelled'] }, description: 'Filter by payment_status enum (exact match)' },
        { in: 'query', name: 'payment_method', schema: { type: 'string', enum: ['cash', 'online'] },          description: 'Filter by payment_method enum (exact match)' },
        { in: 'query', name: 'cleaner_id',     schema: { type: 'string', example: '664f1a2b3c4d5e6f7a8b9c0d' }, description: 'Filter by assigned cleaner\'s CleanerBooking _id (exact match)' },
        { in: 'query', name: 'user_id',        schema: { type: 'string', example: '664f1a2b3c4d5e6f7a8b9c0d' }, description: 'Filter by user_id (exact match)' },
        { in: 'query', name: 'reference_id',   schema: { type: 'string', example: '03-05-2026-00001' },       description: 'Filter by reference_id (exact match)' },
        { in: 'query', name: 'package_name',   schema: { type: 'string', example: 'Premium Package' },        description: 'Filter by package_name (exact match)' },
        ...
        { in: 'query', name: 'updated_from',   schema: { type: 'string',  format: 'date', example: '2026-01-01' }, description: 'Filter updatedAt >= updated_from' },
        { in: 'query', name: 'updated_to',     schema: { type: 'string',  format: 'date', example: '2026-12-31' }, description: 'Filter updatedAt <= updated_to (end of day)' },
        { in: 'query', name: 'is_active',      schema: { type: 'boolean', example: true },                    description: 'Include inactive bookings (default: true = active only)' },
        { in: 'query', name: 'is_delete',      schema: { type: 'boolean', example: false },                   description: 'Include soft-deleted bookings (default: false = non-deleted only)' },
        {
          in: 'query', name: 'sort_by',
          schema: { type: 'string', enum: ['latest_updated', 'oldest_updated', 'latest_created', 'oldest_created'] },
          description: 'Sort order. `latest_updated` returns the most recently updated bookings first. Default: unsorted (natural insertion order).',
        },
      ],
      responses: {
  ...
  '/bookings/{id}': {
    ...
    delete: {
      tags: ['Bookings'],
      summary: 'Soft-delete booking — JWT or API key',
      description: 'Marks the booking as deleted by setting `is_delete` to `true`. The record is not removed from the database and is excluded from `GET /bookings` results by default.',
      security: jwtOrKey,
      parameters: [{ in: 'path', name: 'id', required: true, schema: { type: 'string' } }],
      responses: { 200: { description: 'Booking soft-deleted' }, 404: { description: 'Not found' } },
    },
  },
};
...
export const bookingSchemas = {
  ...
  Booking: {
    ...
    properties: {
      ...
      cleaner_id:          { type: 'string', example: '664f1a2b3c4d5e6f7a8b9c0d', description: '_id of the assigned CleanerBooking record' },
      cleaner_name:          { type: 'string', example: 'Ramesh Kumar', description: 'Joined from the CleanerBooking record via cleaner_id (read-only)', nullable: true },
      cleaner_mobile_number: { type: 'string', example: '+919876543210', description: 'Joined from the CleanerBooking record via cleaner_id (read-only)', nullable: true },
      ...
      is_delete:           { type: 'boolean', example: false, description: 'Soft-delete flag; true when the booking has been deleted' },
  ...
  CreateBookingDto: {
    ...
    properties: {
      ...
      cleaner_id:        { type: 'string', example: '664f1a2b3c4d5e6f7a8b9c0d', description: 'Defaults to "N/A" if omitted' },
      booking_via: {
        type: 'string',
  ...
  UpdateBookingDto: {
    ...
    properties: {
      ...
      cleaner_id:          { type: 'string', example: '664f1a2b3c4d5e6f7a8b9c0d', description: 'ID of the cleaner assigned (optional)' },
      booking_via:         { type: 'string', enum: ['app', 'website', 'laptop', 'whatsapp_to_crm', 'call'] },
      booking_status: {
        type: 'string',
